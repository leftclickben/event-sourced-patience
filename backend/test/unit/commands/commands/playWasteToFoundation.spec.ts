...
import { playWasteToFoundation } from '../../../../src/commands/processors/playWasteToFoundation';

describe('The "play waste to foundation" command', () => {
  const savedEvent: GameEvent = createSampleGameplayEvent(GameEventType.wastePlayedToFoundation);
    ...
    describe('Given the game is in progress', () => {
      describe('Given the waste is not empty and the top card is compatible with the target', () => {
        beforeEach(() => {
          buildTableStateStub.returns({
            ...
            waste: [{ suit: Suit.clubs, value: Value.two, faceUp: true }]
          });
        });
        ...
        describe('When invoked', () => {
          ...
          beforeEach(async () => {
            result = await playWasteToFoundation({
              gameId: 'game-42',
              foundationIndex: 0
            });
          });
          ...
          it('Saves the new event', () => {
            ...
            expect(saveEventStub.firstCall.args).to.deep.equal([
              ...
              { gameId: 'game-42', foundationIndex: 0 }
            ]);
          });
      ...
      describe('Given the card being moved is not compatible with the foundation', () => {
        beforeEach(() => {
          buildTableStateStub.returns({
            foundation: [[{ suit: Suit.clubs, value: Value.ace, faceUp: true }], [], [], []],
            waste: [{ suit: Suit.diamonds, value: Value.jack, faceUp: true }]
          });
